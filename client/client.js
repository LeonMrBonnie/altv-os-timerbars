...
alt.everyTick(() => {
  ...
  native.setScriptGfxAlignParams(
    0.0,
    0.0,
    Helpers.coordsAndSize.gfxAlignWidth,
    Helpers.coordsAndSize.gfxAlignHeight
  );

  let drawY = native.busyspinnerIsOn()
    ? Helpers.coordsAndSize.initialBusySpinnerY
    : Helpers.coordsAndSize.initialY;
  for (const barId in Bars.allBars) {
    let bar = Bars.allBars[barId];
    bar.draw(drawY);
    drawY -= bar._thin
      ? Helpers.coordsAndSize.timerBarThinMargin
      : Helpers.coordsAndSize.timerBarMargin;
  }
